...
def main(argv):
   ...
   # import ipdb; ipdb.set_trace()    
    ...
    (composition_d, contig_lengths,nr_features) = _calculate_composition(fasta, 1000, 4)
    ...
    if False:
        ...
        for (s,sid) in enumerate(sids):
            ...
            distE = np.linalg.norm(comp_arrayP[s,:] - ua)
        
            distG[s] = distE
    
          #  print('%s,%.4f' % (sid,distE))
        #maxDist = np.max(distG)
    
        #for (s,sid) in enumerate(sids):
    
         #   distr = distG[s]/maxDist
       ...
       #     g = int(255*tran_comp_minmax[s,1])
        ...
        #
     #   distU = sp.spatial.distance.cosine(ua, va)
    
    #transform first three dimensions RGB
